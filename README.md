...
### 3) 동시성 이슈 제어 (Concurrency Control)
* **Problem**: 신청 생성 로직은 `검증(Check) -> 저장(Act)`의 흐름을 가집니다. 만약 유저가 버튼을 빠르게 두 번 클릭하여 동시 요청(Race Condition)이 발생할 경우, 트랜잭션이 커밋되기 전이라 두 요청 모두 검증을 통과하여 **중복 신청서가 생성되는 문제**가 발생할 수 있습니다.
* **Solution**: **비관적 락(Pessimistic Lock)**을 적용했습니다. 신청 주체인 `User` 엔티티를 조회할 때 `PESSIMISTIC_WRITE`(`SELECT ... FOR UPDATE`) 락을 걸어, 해당 유저의 모든 트랜잭션을 **물리적으로 순차 처리(Serialize)** 되도록 강제했습니다. 이를 통해 별도의 분산 락 없이도 유저 중복 신청 및 진료 건 중복 매핑을 완벽하게 방지했습니다.
